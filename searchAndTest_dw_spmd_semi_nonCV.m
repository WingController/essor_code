function [results_kfdor,results_fkfdor,results_efkfdor,kfdor_cell,fkfdor_cell,efkfdor_cell] = searchAndTest_dw_spmd_semi_nonCV(sortedData,binSize,K,trainSize,labeledRatio,testSize,runtimes,kerType,I_itermax,lb,ub,saveItr,objFuncName)
...
spmd(runtimes)    
    ...
    [FVr_x,saveProc] = runDE_mae_dw(K,labeledSet,unlabeledSet,membership0,kerType,fkfdorParams_mae,I_itermax,lb,ub,saveItr,objFuncName);
    ...
    lambda = (FVr_x(1:end-1))'; % K*1, fkfdor_downWeight, the last element of evolved vector is the down-weight parameter 
    ...
    dw_factor = FVr_x(end); % the last element of evolved vector is the down-weight parameter
    dw_factor = (dw_factor-lb)/(ub-lb);  % map to [0,1]
    % dw
    [mae_efkfdor,mze_efkfdor] = run_kfdor_fuzzy_downWeight_semi(labeledSet,unlabeledSet,testSet,kerType,fkfdorParams_mae(1),fkfdorParams_mae(2),fkfdorParams_mae(3),membership_new,dw_factor);
