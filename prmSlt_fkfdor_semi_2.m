function [bestmean,beststd,bestu,bestkp,bestc] = prmSlt_fkfdor_semi_2(train,validation,unlabeledSet,membership,K,kerType,Ubasenum,umin,umax,ustep,KPbasenum,KPmin,KPmax,KPstep,Cbasenum,cmin,cmax,cstep,v,metric)
% search best params for fuzzy_kfdor, nonCV
...
parfor vi = 1:v
    ...
    dataSet = [train{vi};unlabeledSet{vi}];
    ...
    for ki = 1:kplen
        ...
        [M,N] = getMandN_fuzzy(dataSet,kerType,kerParams,membership{vi},1);
        for ui = 1:ulen
            u = Ubasenum^X(ui);            
            for ci = 1:clen               
                C = Cbasenum^Z(ci);
