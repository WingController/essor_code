...
% if (S_x.I_no > 0)
%    for k=1:S_x.I_no
%       if (S_x.FVr_oa(k) > S_y.FVr_oa(k))%if just one objective of S_x is less,相等也算赢
%          I_z = 0;
%       end
%    end
% end

% 按顺序比较不同的目标，越靠前的目标拥有越大的决定权
if (S_x.I_no > 0)
   for k=1:S_x.I_no
      if (S_x.FVr_oa(k) < S_y.FVr_oa(k))
          I_z = 1;
          break;
      elseif (S_x.FVr_oa(k) == S_y.FVr_oa(k))
         % I_z = 1; %相等也算赢
          I_z = 0; % equaling is not winning here
      else
          I_z = 0;
          break;
      end
