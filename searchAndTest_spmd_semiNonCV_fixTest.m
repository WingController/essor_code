function [results_kfdor,results_fkfdor,results_efkfdor] = searchAndTest_spmd_semiNonCV_fixTest(sortedData,binSize,K,trainSize,labeledRatio,testSize,test_trainSet,test_testSet,runtimes,kerType,I_itermax,saveItr,objFuncName)
% fix trainSet and test_testSet(test_trainSet,test_testSet) for testing. random generate different [labeled,unlabeled] from trainSet.
...
spmd(runtimes)    
    [labeledSet,unlabeledSet] = partitionTrainset(test_trainSet,labeledRatio);

    %labeledSize = round(trainSize*labeledRatio);
    %[labeledSet,unlabeledSet] = randPartition(test_trainSet,K,labeledSize);
    
    [mae_kfdor,~] = run_kfdor(labeledSet,test_testSet,kerType,kfdorParams_mae(1),kfdorParams_mae(2),kfdorParams_mae(3));
    [~,mze_kfdor] = run_kfdor(labeledSet,test_testSet,kerType,kfdorParams_mze(1),kfdorParams_mze(2),kfdorParams_mze(3));
    ...
    [mae_fkfdor,~] = run_kfdor_fuzzy_semi(labeledSet,unlabeledSet,test_testSet,kerType,fkfdorParams_mae(1),fkfdorParams_mae(2),fkfdorParams_mae(3),membership,1);  % b =1
    [~,mze_fkfdor] = run_kfdor_fuzzy_semi(labeledSet,unlabeledSet,test_testSet,kerType,fkfdorParams_mze(1),fkfdorParams_mze(2),fkfdorParams_mze(3),membership,1);  % b =1

    % efkfdor
    membership0 = membership;
    [FVr_x,saveProc] = runDE_mae(K,labeledSet,unlabeledSet,membership0,kerType,fkfdorParams_mae,I_itermax,saveItr,objFuncName);
    ...
    [mae_efkfdor,mze_efkfdor] = run_kfdor_fuzzy_semi(labeledSet,unlabeledSet,test_testSet,kerType,fkfdorParams_mae(1),fkfdorParams_mae(2),fkfdorParams_mae(3),membership_new,1);
