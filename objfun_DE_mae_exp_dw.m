...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function S_MSE = objfun_DE_mae_exp_dw(FVr_temp, S_struct)
% fkfdor_downWeight, the last element of evolved vector is the down-weight parameter 
% ujk' = (ujk^lk)/sum(ujk^lk)
% 以在验证集上的效果作为fitness
...
lambda = (FVr_temp(1:end-1))'; % K*1, fkfdor_downWeight, the last element of evolved vector is the down-weight parameter 
...
dw_factor = FVr_temp(end); % the last element of evolved vector is the down-weight parameter
dw_factor = (dw_factor-lb)/(ub-lb);  % map to [0,1]
membership = [u_labeled;dw_factor*u_unlabeled_new]; % 新的隶属度矩阵N*K
